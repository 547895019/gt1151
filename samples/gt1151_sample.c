...
static rt_thread_t  gt1151_thread = RT_NULL;
static rt_sem_t     gt1151_sem = RT_NULL;
static rt_device_t  dev = RT_NULL;
static struct       rt_touch_data *read_data;
static struct       rt_touch_info info;

static void gt1151_entry(void *parameter)
{
    ...
    while (1)
    {
        rt_sem_take(gt1151_sem, RT_WAITING_FOREVER);
...
{
    rt_sem_release(gt1151_sem);
    rt_device_control(dev, RT_TOUCH_CTRL_DISABLE_INT, RT_NULL);
    return 0;
}

/* Test function */
int gt1151_sample(const char *name, rt_uint16_t x, rt_uint16_t y)
{
    void *id;
    ...
    gt1151_sem = rt_sem_create("dsem", 0, RT_IPC_FLAG_FIFO);

    if (gt1151_sem == RT_NULL)
    {
        rt_kprintf("create dynamic semaphore failed.\n");
        return -1;
    }

    gt1151_thread = rt_thread_create("thread1",
                                     gt1151_entry,
                                     RT_NULL,
                                     THREAD_STACK_SIZE,
                                     THREAD_PRIORITY,
    ...
    if (gt1151_thread != RT_NULL)
        rt_thread_startup(gt1151_thread);
